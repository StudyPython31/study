...
class CreateChatPage(Base_Page.Action):
    ...
    def searchuid(self,searchtext):
        self.find_ById('com.xxxx:id/ll_voice_chat').click()  #跳到语聊页面

        self.find_ById('com.xxxx:id/ivSearch').click()
        self.excuteAdbShell('adb shell ime set io.appium.android.ime/.UnicodeIME')
        self.find_ById('com.xxxx:id/et_chat_search').click()
        self.find_ById('com.xxxx:id/et_chat_search').send_keys(searchtext)
    ...
    def selectrslt(self, index):
        self.find_ByIndex('com.xxxx:id/rv_search_result', index).click()

    def sendmsg(self,msgs):
        self.find_ById('com.xxxx:id/tvPrivateChat').click()
        self.find_ById('com.xxxx:id/etMsgEdit').send_keys(msgs)
        self.find_ById('com.xxxx:id/btSend').click()
        self.driver.keyevent(4)  # 模拟按下手机键盘的返回,收起键盘


    def getmsg(self,index):
        self.find_ById('com.xxxx:id/ll_message').click()# 跳到消息页面
        msgindex = self.find_ByIndex('com.xxxx:id/tv_private_chat_name', index)
